...
from streaming.data_validation.data_contract_paypal_miller import (
    CONSUMED_FIELDNAMES,
...
OUTPUT_CSV: Final[Path] = OUTPUT_DIR / "consumed_sales_paypal_miller.csv"
...
def process_message(
    ...
    *,
    ...
    LOG.info(f"day_of_week={enriched['day_of_week']}")
...
def consume_messages(
    ...
    *,
    ...
    product_ids = set()
    # Create loop to create set of valid product ids
    for row in read_csv_as_lookup(
        PRODUCTS_CSV, key_field="product_id", value_field="product_name"
    ).items():
    ...
    while consumed_count + skipped_count < MAX_MESSAGES:
        ...
        result = validate_sale_record(
            record=row,
            valid_region_ids=set(region_lookup.keys()),
            valid_product_ids=product_ids,
        )

        if enriched is None or not result.is_valid:
            skipped_count += 1
